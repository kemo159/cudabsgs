...
#ifdef KEYHUNT_CUDA
#include "cuda/keyhunt_cuda_bridge.h"
#endif
...
#ifdef KEYHUNT_CUDA
static void point_to_cuda_bsgs(Point &src, cuda_bsgs_affine_point *dst) {
	for(int i = 0; i < 4; i++) {
		dst->x.v[i] = src.x.bits64[i];
		dst->y.v[i] = src.y.bits64[i];
	}
	dst->infinity = src.isZero() ? 1U : 0U;
	dst->reserved = 0;
}

static void int_to_cuda_bsgs_scalar(Int &src, cuda_bsgs_scalar *dst) {
	for(int i = 0; i < 4; i++) {
		dst->v[i] = src.bits64[i];
	}
}

static uint32_t keyhunt_cuda_xpoint_cycles(void) {
	const char *env = getenv("KEYHUNT_CUDA_XPOINT_CYCLES");
	if(env == NULL || env[0] == 0) {
		return 256;
	}
	char *end = NULL;
...
static uint32_t keyhunt_cuda_center_cycles(void) {
	const char *env = getenv("KEYHUNT_CUDA_CENTER_CYCLES");
	if(env == NULL || env[0] == 0) {
		return 512;
	}
	char *end = NULL;
...
static int keyhunt_cuda_generate_xpoints(void) {
	const char *env = getenv("KEYHUNT_CUDA_GENERATE_XPOINTS");
	return env != NULL && env[0] != 0 && strcmp(env,"0") != 0;
}

static int keyhunt_cuda_double_buffer(void) {
	const char *env = getenv("KEYHUNT_CUDA_DOUBLE_BUFFER");
	return env != NULL && env[0] != 0 && strcmp(env,"0") != 0;
}
...
static const size_t BSGS_XVALUE_RAM = 6;
...
int main(int argc, char **argv)	{
	...
	Int total,pretotal,interval_total,interval_speed,last_total,last_seconds,debugcount_mpz,seconds,div_pretotal,div_interval,int_aux,int_r,int_q,int58;
...
#ifdef KEYHUNT_CUDA
		printf("[+] CUDA uploading BSGS bloom filters and bP table ... ");
		fflush(stdout);
		if(!keyhunt_cuda_bsgs_prepare(bloom_bP,bloom_bPx2nd,bloom_bPx3rd,bPtable,bsgs_m3))	{
			fprintf(stderr,"\n[E] CUDA BSGS prepare failed: %s\n",keyhunt_cuda_last_error());
			exit(EXIT_FAILURE);
		}
		printf("done\n");
		printf("[+] CUDA uploading BSGS search points ... ");
		fflush(stdout);
		{
			cuda_bsgs_affine_point cuda_gsn[CPU_GRP_SIZE / 2];
			cuda_bsgs_affine_point cuda_2gsn;
			for(i = 0; i < CPU_GRP_SIZE / 2; i++) {
				point_to_cuda_bsgs(GSn[i], &cuda_gsn[i]);
			}
			point_to_cuda_bsgs(_2GSn, &cuda_2gsn);
			if(!keyhunt_cuda_bsgs_upload_search_points(cuda_gsn,&cuda_2gsn))	{
				fprintf(stderr,"\n[E] CUDA BSGS search point upload failed: %s\n",keyhunt_cuda_last_error());
				exit(EXIT_FAILURE);
			}
		}
		printf("done\n");
		printf("[+] CUDA uploading BSGS filter points ... ");
		fflush(stdout);
		{
			cuda_bsgs_affine_point *cuda_original = (cuda_bsgs_affine_point*) malloc((size_t)bsgs_point_number * sizeof(cuda_bsgs_affine_point));
			cuda_bsgs_affine_point cuda_amp2[32];
			cuda_bsgs_affine_point cuda_amp3[32];
			checkpointer((void *)cuda_original,__FILE__,"malloc","cuda_original" ,__LINE__ -1 );
			for(i = 0; i < bsgs_point_number; i++) {
				point_to_cuda_bsgs(OriginalPointsBSGS[i], &cuda_original[i]);
			}
			for(i = 0; i < 32; i++) {
				point_to_cuda_bsgs(BSGS_AMP2[i], &cuda_amp2[i]);
				point_to_cuda_bsgs(BSGS_AMP3[i], &cuda_amp3[i]);
			}
			if(!keyhunt_cuda_bsgs_upload_filter_points(cuda_original,(uint32_t)bsgs_point_number,cuda_amp2,cuda_amp3))	{
				fprintf(stderr,"\n[E] CUDA BSGS filter point upload failed: %s\n",keyhunt_cuda_last_error());
				exit(EXIT_FAILURE);
			}
			free(cuda_original);
			cuda_bsgs_scalar cuda_m_double;
			cuda_bsgs_scalar cuda_m2_double;
			cuda_bsgs_scalar cuda_m3;
			cuda_bsgs_scalar cuda_m3_double;
			cuda_bsgs_scalar cuda_order;
			int_to_cuda_bsgs_scalar(BSGS_M_double, &cuda_m_double);
			int_to_cuda_bsgs_scalar(BSGS_M2_double, &cuda_m2_double);
			int_to_cuda_bsgs_scalar(BSGS_M3, &cuda_m3);
			int_to_cuda_bsgs_scalar(BSGS_M3_double, &cuda_m3_double);
			int_to_cuda_bsgs_scalar(secp->order, &cuda_order);
			if(!keyhunt_cuda_bsgs_upload_filter_scalars(&cuda_m_double,&cuda_m2_double,&cuda_m3,&cuda_m3_double,&cuda_order))	{
				fprintf(stderr,"\n[E] CUDA BSGS filter scalar upload failed: %s\n",keyhunt_cuda_last_error());
				exit(EXIT_FAILURE);
			}
		}
		printf("done\n");
	...
	total.SetInt32(0);
	pretotal.SetInt32(0);
	last_total.SetInt32(0);
	last_seconds.SetInt32(0);
	...
	do	{
		...
		if(OUTPUTSECONDS.IsGreater(&ZERO) ){
			...
			if(MPZAUX.IsZero()) {
				...
				interval_total.Set(&total);
				interval_total.Sub(&last_total);
				interval_speed.Set(&seconds);
				interval_speed.Sub(&last_seconds);
				if(interval_speed.IsGreater(&ZERO)) {
					interval_total.Div(&interval_speed);
				}
				str_interval = interval_total.GetBase10();
				
				
				if(pretotal.IsLower(&int_limits[0]))	{
					if(FLAGMATRIX)	{
						sprintf(buffer,"[+] Total %s keys in %s seconds: %s keys/s | inst %s keys/s\n",str_total,str_seconds,str_pretotal,str_interval);
					}
					else	{
						sprintf(buffer,"\r[+] Total %s keys in %s seconds: %s keys/s | inst %s keys/s\r",str_total,str_seconds,str_pretotal,str_interval);
					}
				}
				else	{
					i = 0;
					...
					div_interval.Set(&interval_total);
					div_interval.Div(&int_limits[salir ? i : i-1]);
					str_divinterval = div_interval.GetBase10();
					if(FLAGMATRIX)	{
						sprintf(buffer,"[+] Total %s keys in %s seconds: ~%s %s (%s keys/s) | inst ~%s %s (%s keys/s)\n",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal,str_divinterval,str_limits_prefixs[salir ? i : i-1],str_interval);
					}
					else	{
						if(THREADOUTPUT == 1)	{
							sprintf(buffer,"\r[+] Total %s keys in %s seconds: ~%s %s (%s keys/s) | inst ~%s %s (%s keys/s)\r",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal,str_divinterval,str_limits_prefixs[salir ? i : i-1],str_interval);
						}
						else	{
							sprintf(buffer,"\r[+] Total %s keys in %s seconds: ~%s %s (%s keys/s) | inst ~%s %s (%s keys/s)\r",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal,str_divinterval,str_limits_prefixs[salir ? i : i-1],str_interval);
						}
					...
					free(str_divinterval);

				}
				last_total.Set(&total);
				last_seconds.Set(&seconds);
				printf("%s",buffer);
				fflush(stdout);
				THREADOUTPUT = 0;			
#ifdef _WIN64
...
#ifdef KEYHUNT_CUDA
	enum { cuda_max_hits = 4096 };
	const uint32_t cuda_xpoint_cycles = keyhunt_cuda_xpoint_cycles();
	const uint32_t cuda_xpoint_batch = cuda_xpoint_cycles * CPU_GRP_SIZE;
	unsigned char *cuda_xpoints[2] = {NULL,NULL};
	int cuda_xpoints_pinned[2] = {1,1};
	uint32_t *cuda_cycle_j[2] = {NULL,NULL};
	cuda_bsgs_hit cuda_hits[2][cuda_max_hits];
	cuda_bsgs_probe_workspace *cuda_probe_workspace[2] = {NULL,NULL};
	cuda_bsgs_center_workspace *cuda_center_workspace = NULL;
	for(int cuda_buf = 0; cuda_buf < 2; cuda_buf++) {
		cuda_cycle_j[cuda_buf] = (uint32_t*)malloc((size_t)cuda_xpoint_cycles * sizeof(uint32_t));
		if(!keyhunt_cuda_host_alloc((void **)&cuda_xpoints[cuda_buf],(size_t)cuda_xpoint_batch * 32)) {
			cuda_xpoints_pinned[cuda_buf] = 0;
			cuda_xpoints[cuda_buf] = (unsigned char*)malloc((size_t)cuda_xpoint_batch * 32);
		}
		checkpointer((void *)cuda_xpoints[cuda_buf],__FILE__,"malloc","cuda_xpoints" ,__LINE__ -5 );
		checkpointer((void *)cuda_cycle_j[cuda_buf],__FILE__,"malloc","cuda_cycle_j" ,__LINE__ -3 );
		if(!keyhunt_cuda_bsgs_probe_workspace_create(&cuda_probe_workspace[cuda_buf])) {
			fprintf(stderr,"[E] CUDA BSGS probe workspace failed: %s\n",keyhunt_cuda_last_error());
			exit(EXIT_FAILURE);
		}
	}
	if(keyhunt_cuda_generate_xpoints() &&
		!keyhunt_cuda_bsgs_center_workspace_create(&cuda_center_workspace)) {
		fprintf(stderr,"[E] CUDA BSGS center workspace failed: %s\n",keyhunt_cuda_last_error());
		exit(EXIT_FAILURE);
	}
	const int cuda_use_double_buffer = keyhunt_cuda_double_buffer();
#endif
		...
		*/
		...
		for(k = 0; k < bsgs_point_number ; k++)	{
			if(bsgs_found[k] == 0)	{
...
#ifdef KEYHUNT_CUDA
				if(keyhunt_cuda_generate_xpoints()) {
					const uint32_t cuda_center_cycles = keyhunt_cuda_center_cycles();
					cuda_bsgs_center_hit cuda_center_hits[cuda_max_hits];
					cuda_bsgs_affine_point cuda_seed;
					cuda_bsgs_affine_point cuda_next_seed;
					point_to_cuda_bsgs(startP, &cuda_seed);
					while( j < cycles && bsgs_found[k]== 0 ) {
						uint32_t work_count = cuda_center_cycles;
						if(work_count > (cycles - j)) {
							work_count = cycles - j;
						}
						uint32_t cuda_hit_count = 0;
						if(!keyhunt_cuda_bsgs_probe_center_sequence_workspace(cuda_center_workspace,&cuda_seed,j,work_count,
							cuda_center_hits,cuda_max_hits,&cuda_hit_count,&cuda_next_seed)) {
							fprintf(stderr,"[E] CUDA BSGS center sequence failed: %s\n",keyhunt_cuda_last_error());
							exit(EXIT_FAILURE);
						}
						cuda_seed = cuda_next_seed;
						j += work_count;

						uint32_t cuda_hits_to_process = cuda_hit_count < (uint32_t)cuda_max_hits ? cuda_hit_count : (uint32_t)cuda_max_hits;
						for(uint32_t cuda_h = 0; cuda_h < cuda_hits_to_process && bsgs_found[k] == 0; cuda_h++) {
							uint64_t hit_a64 = (cuda_center_hits[cuda_h].table_index * 1024ULL) + cuda_center_hits[cuda_h].point_index;
							if(hit_a64 > 0xffffffffULL) {
								continue;
							}
							r = bsgs_secondcheck(&base_key,(uint32_t)hit_a64,k,&keyfound);
							if(r)	{
				...
				int cuda_pending[2] = {0,0};
				uint32_t cuda_pending_batch_cycles[2] = {0,0};
				int cuda_active = 0;
				while( j < cycles && bsgs_found[k]== 0 )	{
					if(cuda_use_double_buffer && cuda_pending[cuda_active]) {
						uint32_t cuda_hit_count = 0;
						if(!keyhunt_cuda_bsgs_probe_xpoints_bloom1_finish(cuda_probe_workspace[cuda_active],
							cuda_hits[cuda_active],cuda_max_hits,&cuda_hit_count)) {
							fprintf(stderr,"[E] CUDA BSGS xpoint probe failed: %s\n",keyhunt_cuda_last_error());
							exit(EXIT_FAILURE);
						}
						cuda_pending[cuda_active] = 0;
						uint32_t cuda_hits_to_process = cuda_hit_count < (uint32_t)cuda_max_hits ? cuda_hit_count : (uint32_t)cuda_max_hits;
						for(uint32_t cuda_h = 0; cuda_h < cuda_hits_to_process && bsgs_found[k] == 0; cuda_h++) {
							uint32_t xpoint_index = cuda_hits[cuda_active][cuda_h].xpoint_index;
							uint32_t cycle_index = xpoint_index / CPU_GRP_SIZE;
							uint32_t point_index = xpoint_index % CPU_GRP_SIZE;
							if(cycle_index >= cuda_pending_batch_cycles[cuda_active]) {
								continue;
							}
							uint32_t a = (cuda_cycle_j[cuda_active][cycle_index] * 1024) + point_index;
							r = bsgs_secondcheck(&base_key,a,k,&keyfound);
							if(r)	{
								hextemp = keyfound.GetBase16();
								printf("[+] Thread Key found privkey %s   \n",hextemp);
								point_found = secp->ComputePublicKey(&keyfound);
					...
					uint32_t batch_points = 0;
					while(batch_cycles < cuda_xpoint_cycles && j < cycles && bsgs_found[k] == 0) {
						int i;
						unsigned char *cuda_cycle_xpoints = cuda_xpoints[cuda_active] + ((size_t)batch_points * 32);
						for(i = 0; i < hLength; i++) {
						...
						startP.x.Get32Bytes(cuda_cycle_xpoints + ((CPU_GRP_SIZE / 2) * 32));
						for(i = 0; i<hLength; i++) {
							pp = startP;
							pn = startP;
							...
							pp.x.Get32Bytes(cuda_cycle_xpoints + ((CPU_GRP_SIZE / 2 + (i + 1)) * 32));
							pn.x.Get32Bytes(cuda_cycle_xpoints + ((CPU_GRP_SIZE / 2 - (i + 1)) * 32));
						}
						...
						pn.x.ModAdd(&_p);
						pn.x.ModSub(&GSn[i].x);
						pn.x.Get32Bytes(cuda_cycle_xpoints);

						cuda_cycle_j[cuda_active][batch_cycles] = j;
						batch_points += CPU_GRP_SIZE;
					...
					if(batch_points > 0) {
						if(cuda_use_double_buffer) {
							if(!keyhunt_cuda_bsgs_probe_xpoints_bloom1_submit(cuda_probe_workspace[cuda_active],
								cuda_xpoints[cuda_active],batch_points,cuda_max_hits)) {
								fprintf(stderr,"[E] CUDA BSGS xpoint probe failed: %s\n",keyhunt_cuda_last_error());
								exit(EXIT_FAILURE);
							}
							cuda_pending[cuda_active] = 1;
							cuda_pending_batch_cycles[cuda_active] = batch_cycles;
							cuda_active = 1 - cuda_active;
						}
						else {
							uint32_t cuda_hit_count = 0;
							if(!keyhunt_cuda_bsgs_probe_xpoints_bloom1_workspace(cuda_probe_workspace[cuda_active],
								cuda_xpoints[cuda_active],batch_points,cuda_hits[cuda_active],
								cuda_max_hits,&cuda_hit_count)) {
								fprintf(stderr,"[E] CUDA BSGS xpoint probe failed: %s\n",keyhunt_cuda_last_error());
								exit(EXIT_FAILURE);
							}
							uint32_t cuda_hits_to_process = cuda_hit_count < (uint32_t)cuda_max_hits ? cuda_hit_count : (uint32_t)cuda_max_hits;
				...
				for(int cuda_finish = 0; cuda_finish < 2 && bsgs_found[k] == 0; cuda_finish++) {
					int cuda_buf = (cuda_active + cuda_finish) & 1;
					if(cuda_pending[cuda_buf]) {
						uint32_t cuda_hit_count = 0;
						if(!keyhunt_cuda_bsgs_probe_xpoints_bloom1_finish(cuda_probe_workspace[cuda_buf],
							cuda_hits[cuda_buf],cuda_max_hits,&cuda_hit_count)) {
							fprintf(stderr,"[E] CUDA BSGS xpoint probe failed: %s\n",keyhunt_cuda_last_error());
							exit(EXIT_FAILURE);
						}
						cuda_pending[cuda_buf] = 0;
						uint32_t cuda_hits_to_process = cuda_hit_count < (uint32_t)cuda_max_hits ? cuda_hit_count : (uint32_t)cuda_max_hits;
						for(uint32_t cuda_h = 0; cuda_h < cuda_hits_to_process && bsgs_found[k] == 0; cuda_h++) {
							uint32_t xpoint_index = cuda_hits[cuda_buf][cuda_h].xpoint_index;
							uint32_t cycle_index = xpoint_index / CPU_GRP_SIZE;
							uint32_t point_index = xpoint_index % CPU_GRP_SIZE;
							if(cycle_index >= cuda_pending_batch_cycles[cuda_buf]) {
								continue;
							}
							uint32_t a = (cuda_cycle_j[cuda_buf][cycle_index] * 1024) + point_index;
							r = bsgs_secondcheck(&base_key,a,k,&keyfound);
							if(r)	{
								hextemp = keyfound.GetBase16();
								printf("[+] Thread Key found privkey %s   \n",hextemp);
...
thread_bsgs_done:
	...
	for(int cuda_buf = 0; cuda_buf < 2; cuda_buf++) {
		keyhunt_cuda_bsgs_probe_workspace_destroy(cuda_probe_workspace[cuda_buf]);
		if(cuda_xpoints_pinned[cuda_buf]) {
			keyhunt_cuda_host_free(cuda_xpoints[cuda_buf]);
		}
		else {
			free(cuda_xpoints[cuda_buf]);
		}
		free(cuda_cycle_j[cuda_buf]);
	}
#endif
	ends[thread_number] = 1;
	return NULL;
